# Hi, I'm Nate Johnson

Senior Product Designer and Creative Technologist building AI-first products, design systems, and interactive prototypes.

I've designed products and platforms for Microsoft, Xbox, Google, Amazon, EA SPORTS, Whatnot, and REI. Today I combine product design, AI, and code to turn strategy into working systems.

## Featured work

| Project | Live experience | What it demonstrates |
| --- | --- | --- |
| [Plantasonic](https://github.com/nate-thousand/plantasonic) | [Launch](https://www.plantasonic.xyz/) | A generative audiovisual instrument built from a reusable platform, design system, sound engine, and ASCII visual engine |
| [Signal 9](https://github.com/nate-thousand/signal-9-live) | [Launch](https://signal-9-live.vercel.app/) | An AI-first broadcast terminal combining structured narrative, music, state, and real-time visuals |
| [ASCII Visual Engine](https://github.com/nate-thousand/ascii-visual-engine) | [Explore](https://ascii-visual-engine.vercel.app/) | A documented TypeScript framework for expressive real-time ASCII systems |
| [Plantasonic Design System](https://github.com/nate-thousand/plantasonic-design-system) | [Explore](https://plantasonic-design-system.vercel.app/) | Tokens, components, application shells, documentation, and accessible interaction patterns |
| [Treat Life](https://github.com/nate-thousand/treat-life) | [Launch](https://treat-life.vercel.app/) | A configuration-driven brand prototype and AI-first design system |

## The Plantasonic ecosystem

```text
Plantasonic experience
├── Plantasonic Platform
├── Plantasonic Design System
├── ASCII Visual Engine
└── Plantasia Sound Engine

Signal 9
└── Built on the same platform and engines
```

## How I work

- Start with a product thesis and interaction model
- Build reusable systems instead of isolated screens
- Use AI as a collaborative design and development tool
- Keep deterministic product behavior, accessibility, and human judgment explicit
- Connect design decisions to working code and deployed experiences

## Skills

**Product design:** Product strategy, UX, visual design, interaction design, information architecture

**Design systems:** Figma, design tokens, component libraries, documentation, accessibility

**Development:** TypeScript, React, Next.js, HTML, CSS, GitHub, Vercel

**AI:** OpenAI, Claude, Cursor, GitHub Copilot, AI-assisted product development

## Selected experience

Microsoft · Xbox · Google · Amazon · EA SPORTS · Whatnot · REI

## Portfolio

- [Current AI-first portfolio](https://natejohnson-portfolio.xyz/) — access controlled
- [Selected work archive](https://www.natethousand.com/)

## Connect

- [LinkedIn](https://www.linkedin.com/in/neight1000/)
